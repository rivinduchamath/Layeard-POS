...
public class OrderDAOImpl implements OrderDAO {
    ...
    public boolean existByCustomerId(String customerId) throws Exception {
        ResultSet rs = CrudUtil.execute("SELECT * FROM `Order` WHERE customerId=?", customerId);
        return rs.next();
    }

    @Override
    public boolean existsByItemId(String itemCode) throws Exception {
        ResultSet rs=CrudUtil.execute("SELECT * FROM `Order` WHERE itemCode=?",itemCode);
        return rs.next();
    }
